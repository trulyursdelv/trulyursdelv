const rss = {
  write(content) {
    const title = "trulyursdelv feed";
    const desc = "trulyursdelv github updates (@trulyursdelv)";
    const prefix = `<?xml version="1.0" encoding="UTF-8" ?>\n<rss><channel><title><![CDATA[ ${title} ]]></title><link>https://github.com/trulyursdelv</link><description><![CDATA[ ${desc} ]]></description>`;
...
const git = {
  async repo() {
    const req = await fetch("https://api.github.com/users/trulyursdelv/repos");
    ...
    return raw.map(repo => {
      return {
        ...
        desc: "@trulyursdelv created a new repository" + (repo.description == null ? "" : " | " + repo.description),
        img: `https://og-theta.vercel.app/api/general?siteName=${encodeURIComponent(repo.full_name)}&amp;description=${encodeURIComponent(repo.description || "@trulyursdelv created a new repository")}&amp;theme=dark&amp;logo=https://crebin.vercel.app/static/avatar.png&amp;logoWidth=120`,
  ...
  async retrieve() {
    ...
    repo.unshift({
      ...
      title: "@trulyursdelv",
      desc: "inactive | programming stuff xoxo",
